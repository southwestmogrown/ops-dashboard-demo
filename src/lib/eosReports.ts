...
function getNoteValueIfEnabled(
  enabled: boolean,
  value: string,
): string {
...
function generateEOSCSV(
  ...
  const rows = activeLines.map(({ lineKey, line }) => {
    ...
    return [
      ...
      l.downtimeUnitsLost, l.downtimeCount, l.openDowntimeCount, l.latestDowntimeReason, l.lineNotes,
    ].join(",");
  });
  const meta = [
    "End of Shift Report",
    ...
    `Resolved During Shift:,${getNoteValueIfEnabled(data.notes.resolvedDuringShiftEnabled, data.notes.resolvedDuringShift)}`,
    `Open Items Next Shift:,${getNoteValueIfEnabled(data.notes.openItemsNextShiftEnabled, data.notes.openItemsNextShift)}`,
    `Equipment Concerns:,${getNoteValueIfEnabled(data.notes.equipmentConcernsEnabled, data.notes.equipmentConcerns)}`,
...
function generateLineStatusCSV(
  ...
  const rows = activeLines.map(({ vsId, vsName, lineKey, line }) => {
    ...
    return [
      ...
      l.downtimeMinutes || "0", l.downtimeUnitsLost || "0", l.downtimeCount || "0", l.openDowntimeCount || "0",
...
export function generateEmailBody(
  ...
  const lineRows = activeLines
    ...
    Downtime: ${l.downtimeMinutes || "0"} min  |  Lids Lost: ${l.downtimeUnitsLost || "0"}  |  Stops: ${l.downtimeCount || "0"}  |  Open Stops: ${l.openDowntimeCount || "0"}  |  Latest Stop: ${l.latestDowntimeReason || "—"}
