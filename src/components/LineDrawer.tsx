...
const DOWNTIME_BADGE_COLORS: Record<string, string> = {
  ...
  "material-shortage":  "bg-status-amber/20 text-status-amber border-status-amber/30",
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <span className="material-symbols-outlined text-[#e1e2ec]/10 text-4xl mb-2">event_busy</span>
        <p className="text-xs text-[#e1e2ec]/30">No downtime logged this shift</p>
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-1">
              <div className="flex flex-col gap-1">
                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-sm border text-[9px] font-bold uppercase tracking-widest ${DOWNTIME_BADGE_COLORS[entry.reason] ?? DOWNTIME_BADGE_COLORS["other"]}`}>
                  {DOWNTIME_REASON_LABELS[entry.reason] ?? entry.reason}
                </span>
                {entry.notes && (
                  <p className="text-[10px] text-[#e1e2ec]/50 italic">{entry.notes}</p>
                )}
              </div>
              <div className="text-right shrink-0">
                {isOngoing ? (
                  <span className="text-[10px] font-bold text-status-red animate-pulse">ONGOING</span>
                ) : (
            ...
            <div className="mt-1.5 w-full bg-surface-highest rounded-full h-1 overflow-hidden">
              <div
                ...
                style={{ width: isOngoing ? "100%" : `${Math.min(100, (durationMin / 120) * 100)}%` }}
              />
            </div>
...
export default function LineDrawer({
  ...
  const statusLabel = (() => {
    ...
    if (!mesState?.schedule) {
      return { text: "NO SCHEDULE", bg: "bg-slate-500/20", textColor: "text-slate-400", border: "border-slate-500/30" };
    }
    if (paceRatio !== null && paceRatio < 0.75) {
      const primary = reasons[0] ?? "Behind pace";
      return { text: `STOPPED — ${primary}`, bg: "bg-status-red/20", textColor: "text-status-red", border: "border-status-red/30" };
    }
    if (reasons.length > 0) {
      const primary = reasons[0] ?? "At risk";
      return { text: `AT RISK — ${primary}`, bg: "bg-status-amber/20", textColor: "text-status-amber", border: "border-status-amber/30" };
    }
    return { text: "ON TRACK", bg: "bg-status-green/20", textColor: "text-status-green", border: "border-status-green/30" };
  })();

  // ─── Contact info ────────────────────────────────────────────────────────────
  const operatorName     = line ? adminConfig?.[line.id]?.operatorName : undefined;
  const teamLeadContact  = line ? adminConfig?.[line.id]?.teamLeadContact : undefined;
  const hasContact = operatorName || teamLeadContact;
  ...
  const hasMesHourly = mesState && Object.keys(mesState.hourlyOutput).length > 0;
  ...
  const hourlyTargetValue = useMemo(() => {
    ...
    const rows = getHourlyTargets(line.target, shift, mesState?.hourlyOutput ?? {});
    const nonBreakRows = rows.filter((r) => !r.isBreak && r.planned > 0);
    if (nonBreakRows.length === 0) return null;
    return Math.round(nonBreakRows.reduce((s, r) => s + r.planned, 0) / nonBreakRows.length);
  ...
  const etaInfo = (() => {
    if (!mesState || mesState.remainingOnOrder <= 0 || shiftProgress.elapsedHours < 0.25) return null;
        ...
        {line && (
          <>
              ...
              <div className="flex gap-4 px-5 py-2 border-b border-border/50 bg-background/50 shrink-0">
                {operatorName && (
                  <div className="flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-[#e1e2ec]/30 text-sm">person</span>
                    <span className="text-[10px] text-[#e1e2ec]/50">{operatorName}</span>
                  </div>
                )}
                {teamLeadContact && (
                  <div className="flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-[#e1e2ec]/30 text-sm">contact_phone</span>
                    <span className="text-[10px] text-[#e1e2ec]/50">{teamLeadContact}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-5 gap-3 p-5 border-b border-border shrink-0">
              <div>
                <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-1 font-bold">Output</p>
                <p className="text-xl font-semibold font-['Space_Grotesk',sans-serif]">{line.output}</p>
                <p className="text-xs text-[#e1e2ec]/40">/ {line.target}</p>
              </div>
              <div>
                <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-1 font-bold">OEE</p>
                <p className={`text-xl font-semibold font-['Space_Grotesk',sans-serif] ${getOeeColor(line.oee * 100)}`}>
                  {(line.oee * 100).toFixed(1)}%
                </p>
              </div>
              <div>
                <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-1 font-bold">HPU</p>
                <p className={`text-xl font-semibold font-['Space_Grotesk',sans-serif] ${getHpuColor(line.hpu)}`}>
                  {line.hpu.toFixed(2)}
                </p>
              </div>
              <div>
                <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-1 font-bold">C/O</p>
                <p className="text-xl font-semibold font-['Space_Grotesk',sans-serif]">{line.changeovers}</p>
              </div>
              <div>
                <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-1 font-bold">Pace</p>
                {linePace !== null ? (
                  <>
            ...
            {mesState?.schedule && (
              <div className="grid grid-cols-4 gap-3 px-5 py-3 bg-background border-b border-border shrink-0">
                <div>
                  <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-0.5 font-bold">Active Order</p>
                  <p className="text-sm font-mono text-[#e1e2ec] truncate">
                    {mesState.currentOrder ?? <span className="text-status-green">All complete</span>}
                  </p>
                </div>
                <div>
                  <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-0.5 font-bold">Rem. on Order</p>
                  <p className="text-sm text-[#e1e2ec]">{mesState.remainingOnOrder}</p>
                </div>
                <div>
                  <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-0.5 font-bold">ETA</p>
                  {etaInfo ? (
                    <p className="text-sm text-[#e1e2ec] font-semibold">
                      {etaInfo.etaTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </p>
                  ) : (
                ...
                <div>
                  <p className="text-[9px] text-[#e1e2ec]/40 uppercase tracking-widest mb-0.5 font-bold">Rem. on Sheet</p>
                  <p className="text-sm text-[#e1e2ec]">{mesState.remainingOnRunSheet}</p>
                </div>
              </div>
            )}
                ...
                <div className="p-5 space-y-6">
                    ...
                    <>
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-[#e1e2ec]/40 mb-3">
                          Hourly Output
                          {hasMesHourly ? (
                            <span className="ml-2 text-accent/70 normal-case font-normal">· live MES</span>
                          ) : (
                            <span className="ml-2 text-[#e1e2ec]/20 normal-case font-normal">· waiting for output...</span>
                          )}
                        </p>
                        {hourlyBarData.length > 0 ? (
                          <ResponsiveContainer width="100%" height={160}>
                            <BarChart data={hourlyBarData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  {activeTab === "oee" && line && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-surface-high rounded-sm p-4 flex flex-col items-center">
                        ...
                        <p className={`text-5xl font-bold font-['Space_Grotesk',sans-serif] ${getOeeColor(line.oee)}`}>
                          {line.oee.toFixed(1)}%
                        </p>
                        <p className="text-[10px] text-[#e1e2ec]/30 mt-1">A × P × Q</p>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div className="bg-surface-high rounded-sm p-3 flex flex-col items-center">
                          ...
                          <p className={`text-2xl font-bold font-['Space_Grotesk',sans-serif] ${line.availability >= 90 ? "text-status-green" : line.availability >= 80 ? "text-status-amber" : "text-status-red"}`}>
                            {line.availability.toFixed(1)}%
                          </p>
                          <p className="text-[9px] text-[#e1e2ec]/30 mt-1 text-center">
                            {line.availability >= 90 ? "Good uptime" : "Downtime losses"}
                          </p>
                        </div>
                        <div className="bg-surface-high rounded-sm p-3 flex flex-col items-center">
                          <p className="text-[9px] font-black uppercase tracking-widest text-[#e1e2ec]/40 mb-2">
                          ...
                          <p className={`text-2xl font-bold font-['Space_Grotesk',sans-serif] ${line.performance >= 90 ? "text-status-green" : line.performance >= 80 ? "text-status-amber" : "text-status-red"}`}>
                            {line.performance.toFixed(1)}%
                          </p>
                          <p className="text-[9px] text-[#e1e2ec]/30 mt-1 text-center">
                            {line.performance >= 90 ? "At full speed" : "Speed losses"}
                          </p>
                        </div>
                        <div className="bg-surface-high rounded-sm p-3 flex flex-col items-center">
                          <p className="text-[9px] font-black uppercase tracking-widest text-[#e1e2ec]/40 mb-2">
                          ...
                          <p className={`text-2xl font-bold font-['Space_Grotesk',sans-serif] ${getFpyColor(line.fpy)}`}>
                      ...
                      <div className="space-y-1.5 text-[10px] text-[#e1e2ec]/30 leading-relaxed">
                        <p><span className="text-[#e1e2ec]/50 font-medium">Availability</span> — ratio of run time vs planned shift time. Accounts for downtime logged in M8.</p>
                        <p><span className="text-[#e1e2ec]/50 font-medium">Performance</span> — actual UPH vs standard rate (target ÷ shift hours). Capped at 100%.</p>
                        <p><span className="text-[#e1e2ec]/50 font-medium">Quality</span> — first-pass yield from scrap log. Kicked lids reduce FPY; scrapped panels do not.</p>
                      </div>
                    </div>
                  )}
                  ...
                  {activeTab === "hpu" && (
                    <div>
                      <p className="text-[9px] font-black uppercase tracking-widest text-[#e1e2ec]/40 mb-3">
                        HPU Trend
                        <span className="ml-2 text-vs2/60 normal-case font-normal">· from scan log</span>
                      </p>
                  ...
                  {activeTab === "downtime" && (
                    <div>
                      <p className="text-[9px] font-black uppercase tracking-widest text-[#e1e2ec]/40 mb-3">
                        Downtime Log
                        <span className="ml-2 text-vs2/60 normal-case font-normal">· this shift</span>
